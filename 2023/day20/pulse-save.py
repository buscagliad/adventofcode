...
class Conjunction:
	...
	def input(self, frommod):
		self.needinput += 1
		self.cares.append([frommod, False])
	...
	def outpulse(self, f):
		...
		self.lastSent = self.toSend
		self.rcvd.add(f)
		#if not len(self.rcvd) == self.needinput:
		#	print(self.rcvd)
		#	return []
		for s in self.sendto:
			s[2] = self.toSend
