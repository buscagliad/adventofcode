...
def process(line, debug = False):
    global minx, miny, maxy, maxx, zero
    xi = line.find('-x') + 2
    yi = line.find('-y') + 2
    z = line.find(' ')
    q = line[z:].split('T ')
    ...
    for y in range(miny, maxy+1):
        for vx in range(minx, maxx+1):
            for vy in range(miny, maxy+1):
                ...
                if viable(disks[(x,y)], disks[(vx, vy)]):
                    viable_count += 1
print("Part 1: Number of viable node pairs: ", viable_count)  #433

print(zero, disks[zero])
zero_disk = disks[zero]
#
# disks [0] = size, [1] = used  [2] = avail
# steps, zero (x,y), data (x,y)
#
ok = 0
bad = 0
for x in range(minx, maxx+1):
    print()
    for y in range(miny, maxy+1):
        if disks[(x,y)][1] <= zero_disk[0] :
            if x == 0 and y == 0 : print('I', end="")
            elif disks[(x,y)][1] == 0: print('O', end="")
            elif x == maxx and y == 0 : print('G', end="")
            else: print('.', end="")
            ok += 1
        else:
            print('X', end="")
            bad += 1
            #print("Bad: ", (x,y), " size: ", disks[(x,y)][1])
print()
print("ok: ", ok, " bad: ", bad)
