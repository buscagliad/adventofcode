...
In case the Elves get hungry and need extra snacks, they need to know which Elf to ask: theyd like to know how many 
...
{
	int max = elves[0];
	for (int i = 1; i < (int)elves.size(); i++)
	   if (max < elves[i]) max = elves[i];
	return max;
}
...
int main()
{
	...
	for (size_t i = 0; i < elves.size(); i++)
	    printf("%d   %d\n", (int)i, elves[i]);
	sort(elves.begin(), elves.end(), big);
	printf("Max value is: %d\n", elves[0]);
	printf("Top three value is: %d\n", elves[0]+elves[1]+elves[2]);
